...
export default {
        ...
        res
          .filter(
            item =>
              ...
              new Date(item.filedate).getTime() < moment('', 'MM/DD/YYYY').day(0)
          )
          .forEach(item =>
            array.push({
  ...
  handleDates(data) {
    ...
    const sortedDates = [...dateArray]
      .filter(
        date => new Date(date).getTime() >= new Date('01/01/2020').getTime()
      )
      .sort((a, b) => sortByDate(a, b));
    const sortedMonths = [
      ...new Set(
        sortedDates.map(date =>
          moment(date, 'MM/DD/YYYY').startOf('month').format('MM/DD/YYYY')
        )
      )
    ];
    const start = sortedDates[0];
    const end = moment(sortedDates[sortedDates.length - 1], 'MM/DD/YYYY').endOf('week');
    ...
    sortedMonths
      ...
      // )
      .forEach(date => {
        const dateText = moment(date, 'MM/DD/YYYY').format('MMMM YYYY');
